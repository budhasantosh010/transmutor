...
VARIANT_COMMANDS = {
    ...
    "v837aq": [
        *[[sys.executable, "-m", "experiments.v837_primitive_invention.v837aq.run_pipeline", "--stage", stage] for stage in ("source","reality","operator","response-tensors","localize","composition","predictive","meta","freeze","heldout","robustness","analyze")],
    ],
}
...
def enforce_variant_guard(variant: str) -> None:
    ...
    if variant == "v837an":
        ...
        return
    if variant == "v837ao":
        from experiments.v837_primitive_invention.v837ao.authorization import assert_authorized
        authorization = assert_authorized()
        if authorization.get("authorized") is not True or authorization.get("v837an_diagnosis") != "GENERAL_CAUSAL_LATENT_PRIMITIVE_PATTERN":
            raise SystemExit("V837ao blocked: V837an did not authorize latent primitive canonicalization")
        if authorization.get("fresh_audit_consumed") is not False or authorization.get("primitives_promoted") != 0 or authorization.get("v838_started") is not False:
    ...
    if variant == "v837ap":
        ...
        return
    if variant == "v837aq":
        from experiments.v837_primitive_invention.v837aq.authorization import assert_authorized
        authorization = assert_authorized()
        if authorization.get("authorized") is not True or authorization.get("v837ap_diagnosis") != "DECODABLE_LOW_DIMENSIONAL_STATE_NOT_CAUSALLY_CLOSED":
            raise SystemExit("V837aq blocked: V837ap did not authorize program-level causal operator localization")
        if authorization.get("v837ap_next_program") != "V837aq_PROGRAM_LEVEL_CAUSAL_OPERATOR_LOCALIZATION":
            raise SystemExit("V837aq blocked: predecessor next-program contract changed")
        if authorization.get("fresh_audit_consumed") is not False or authorization.get("primitive_archive_population") is not False or authorization.get("primitives_promoted") != 0 or authorization.get("v838_started") is not False:
            raise SystemExit("V837aq blocked: fresh-audit/archive/V838 lock changed")
        return
...
def main() -> int:
    ...
    elif args.variant == "v837aq":
        if args.regime or args.analyze:
            raise SystemExit("--regime/--analyze are only valid with --variant v837ai")
        if args.stage:
            commands = [[sys.executable, "-m", "experiments.v837_primitive_invention.v837aq.run_pipeline", "--stage", args.stage]]
    elif args.regime or args.analyze or args.stage:
        raise SystemExit("--regime/--analyze are only valid with --variant v837ai; --stage is valid only with --variant v837aj, v837al, v837am, v837an, v837ao, v837ap, or v837aq")
